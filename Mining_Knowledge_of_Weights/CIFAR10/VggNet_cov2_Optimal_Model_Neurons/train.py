...
def model_prepare(work):
    ...
    # best_acc = torch.load('./best_acc/flod.t7')
    # net.load_state_dict(best_acc['net'])
...
def train(epoch, dataloader, net, optimizer0, criterion0, optimizer1, criterion1, vali=True):
    ...
    for batch_id, (inputs, targets) in enumerate(dataloader):
        ...
        outputs, gate_prt0, gate_prt1, gate_prt2 = net(inputs, epoch)
        if epoch % 2 == 0:
            gate0 = gate_prt0
            gate1 = gate_prt1
            gate2 = gate_prt2
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, start_epoch+args.max_epoch):
        ...
        if lr0 < 5e-3 and lr1 < 5e-3:
            ...
            plt.savefig("VggNet_cov2_First_CE.jpg")
            plt.figure(2)
            plt.subplot(1, 2, 1)
            plt.xlabel('epoch')
            plt.ylabel('train loss')
            ...
            plt.savefig("VggNet_cov2_Second_CE.jpg")
            plt.show()
            plt.show()
            print('OVER')
            break
