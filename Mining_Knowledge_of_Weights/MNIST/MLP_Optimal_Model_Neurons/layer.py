...
class Fully_Connect0(nn.Module):
    def __init__(self, in_features, out_features, bias=False):
        ...
        self.row_weights0 = Parameter(torch.Tensor(32, 1, 1, 300))
    ...
    def forward(self, input, epoch):
        if self.training:
            if epoch % 2 == 0:
                ...
                layer50 = F.relu(self.dw_bn5(nn.functional.conv2d(layer41, self.dw_weights5, stride=1, padding=1, bias=None)))  # [1, 256, 24, 9]
                ...
                layer110 = F.relu(self.bn0(nn.functional.conv2d(layer102, self.weights0, stride=1, padding=1, bias=None)))
                layer12 = F.relu(self.row_bn0(nn.functional.conv2d(layer110, self.row_weights0, stride=1, bias=None)))
                layer_out = F.relu(self.row_bn1(nn.functional.conv2d(layer12, self.row_weights1, stride=1, bias=None)))  # [1, 1, 784, 1]
...
class Fully_Connect1(nn.Module):
    def __init__(self, in_features, out_features, bias=False):
        ...
        self.up_sample1 = Parameter(torch.Tensor(64, 32, 3, 3))
    ...
    def forward(self, input, epoch):
        if self.training:
            if epoch % 2 == 0:
                ...
                layer30 = F.relu(self.dw_bn3(nn.functional.conv2d(layer21, self.dw_weights3, stride=1, padding=1, bias=None)))
                layer31 = self.maxpool(layer30)
                layer40 = F.relu(self.dw_bn4(nn.functional.conv2d(layer31, self.dw_weights4, stride=1, padding=1, bias=None)))  # [1, 128, 18, 6]
                ...
                layer12 = F.relu(self.row_bn0(nn.functional.conv2d(layer110, self.row_weights0, stride=1, bias=None)))
                layer_out = F.relu(self.row_bn1(nn.functional.conv2d(layer12, self.row_weights1, stride=1, bias=None)))  # [1, 1, 300, 1]
                gate_prt = layer_out.view(self.in_features, 1)
                gate = layer_out.view(1, self.in_features).expand(input.size(0), self.in_features)
                new_input = input.mul(gate)
                output = new_input.mm(self.weights)
    ...
    def forward(self, input, epoch):
        if self.training:
            if epoch % 2 == 0:
                ...
                layer40 = F.relu(self.dw_bn4(nn.functional.conv2d(layer31, self.dw_weights4, stride=1, padding=1, bias=None)))  # [1, 128, 6, 2]
                layer60 = F.relu(self.up_bn00(nn.functional.conv_transpose2d(layer40, self.up_sample0, stride=2, bias=None)))
                layer61 = torch.cat((layer30, layer60), dim=1)
                layer62 = F.relu(self.up_bn01(nn.functional.conv2d(layer61, self.up_weights0, stride=1, padding=1, bias=None)))
                layer70 = F.relu(self.up_bn10(nn.functional.conv_transpose2d(layer62, self.up_sample1, stride=2, bias=None)))
                ...
                layer110 = F.relu(self.bn0(nn.functional.conv2d(layer92, self.weights0, stride=1, padding=1, bias=None)))
                layer12 = F.relu(self.row_bn0(nn.functional.conv2d(layer110, self.row_weights0, stride=1, bias=None)))
                layer_out = F.relu(self.row_bn1(nn.functional.conv2d(layer12, self.row_weights1, stride=1, bias=None)))  # [1, 1, 100, 1]
                gate_prt = layer_out.view(self.in_features, 1)
                gate = layer_out.view(1, self.in_features).expand(input.size(0), self.in_features)
                new_input = input.mul(gate)
                output = new_input.mm(self.weights)
