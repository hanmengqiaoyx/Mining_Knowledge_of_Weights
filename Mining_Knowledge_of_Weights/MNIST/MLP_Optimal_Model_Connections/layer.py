    ...
    def forward(self, input, epoch):
        if self.training:
            if epoch % 2 == 0:
                ...
                layer_out = F.relu(self.bn0(nn.functional.conv2d(layer102, self.weights0, stride=1, padding=1, bias=None)))  # [1, 1, 784, 300]
    ...
    def forward(self, input, epoch):
        if self.training:
            if epoch % 2 == 0:
                ...
                layer92 = F.relu(self.up_bn31(nn.functional.conv2d(layer91, self.up_weights3, stride=1, padding=1, bias=None)))
                layer_out = F.relu(self.bn0(nn.functional.conv2d(layer92, self.weights0, stride=1, padding=1, bias=None)))  # [1, 1, 300, 100]
                gate_prt = layer_out.view(self.in_features*self.out_features, 1)
                gate = layer_out.view(self.in_features, self.out_features)
                new_weights = self.weights.mul(gate)
                output = input.mm(new_weights)
    ...
    def forward(self, input, epoch):
        if self.training:
            if epoch % 2 == 0:
                ...
                layer91 = torch.cat((layer00, layer90), dim=1)
                layer92 = F.relu(self.up_bn31(nn.functional.conv2d(layer91, self.up_weights3, stride=1, padding=1, bias=None)))
                layer_out = F.relu(self.bn0(nn.functional.conv2d(layer92, self.weights0, stride=1, padding=1, bias=None)))  # [1, 1, 100, 10]
                gate_prt = layer_out.view(self.in_features * self.out_features, 1)
                gate = layer_out.view(self.in_features, self.out_features)
                new_weights = self.weights.mul(gate)
                output = input.mm(new_weights)
