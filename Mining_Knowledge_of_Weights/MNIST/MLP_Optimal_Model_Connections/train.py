...
def model_prepare(work):
    ...
    scheduler0 = torch.optim.lr_scheduler.MultiStepLR(optimizer0, milestones=[50, 75, 100], gamma=0.1, last_epoch=-1)
    scheduler1 = torch.optim.lr_scheduler.MultiStepLR(optimizer1, milestones=[50, 75, 100], gamma=0.1, last_epoch=-1)
    # scheduler0 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer0, mode='min', factor=0.1, patience=30, verbose=True, threshold=1e-4, threshold_mode='rel')
    # scheduler1 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer1, mode='min', factor=0.1, patience=30, verbose=True, threshold=1e-4, threshold_mode='rel')
...
def train(epoch, dataloader, net, optimizer0, criterion0, optimizer1, criterion1, vali=True):
    ...
    for batch_id, (inputs, targets) in enumerate(dataloader):
        ...
        outputs, f_gate0, f_gate1, f_gate2 = net(inputs, epoch)
        if epoch % 2 == 0:
            gate0 = f_gate0
            gate1 = f_gate1
            gate2 = f_gate2
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, start_epoch+args.max_epoch):
        train_loss0, train_acc0, train_loss1, train_acc1, gate0, gate1, gate2 = train(epoch, trainloader, net, optimizer0, criterion0, optimizer1, criterion1)
        scheduler0.step()
        scheduler1.step()
