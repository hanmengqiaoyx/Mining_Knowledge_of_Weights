...
import os
import argparse
from model import LeNet5_800_500
...
def model_prepare(work):
    ...
    scheduler0 = torch.optim.lr_scheduler.MultiStepLR(optimizer0, milestones=[50, 75, 100], gamma=0.1)
    scheduler1 = torch.optim.lr_scheduler.MultiStepLR(optimizer1, milestones=[50, 75, 100], gamma=0.1)
    ...
    return net, optimizer0, optimizer1, scheduler0, scheduler1, criterion0, criterion1
...
def train(epoch, dataloader, net, optimizer0, criterion0, optimizer1, criterion1, vali=True):
    ...
    for batch_id, (inputs, targets) in enumerate(dataloader):
        ...
        outputs, c_gate0, c_gate1, f_gate0, f_gate1 = net(inputs, epoch)
        if epoch % 2 == 0:
            gate0 = c_gate0
            gate1 = c_gate1
            gate2 = f_gate0
...
if __name__ == '__main__':
    ...
    net, optimizer0, optimizer1, scheduler0, scheduler1, criterion0, criterion1 = model_prepare(work)
    ...
    for epoch in range(start_epoch, start_epoch+args.max_epoch):
        train_loss0, train_acc0, train_loss1, train_acc1, gate0, gate1, gate2, gate3 = train(epoch, trainloader, net, optimizer0, criterion0, optimizer1, criterion1)
        scheduler0.step()
        scheduler1.step()
        ...
        if epoch == args.max_epoch-1:
            ...
            plt.savefig("LeNet5_First_CE.jpg")
            plt.figure(2)
            plt.subplot(1, 2, 1)
            plt.xlabel('epoch')
            plt.ylabel('train loss')
            ...
            plt.savefig("LeNet5_Second_CE.jpg")
            plt.show()
            print('OVER')
            break
        else:
